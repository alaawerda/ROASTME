...
export default function ChatMessage({ message, timeFormat }: ChatMessageProps) {
  ...
  return (
    <div className={`flex ${isUser ? 'justify-end' : 'justify-start'} mb-3 md:mb-4 animate-in`}>
      <div className={`flex items-start space-x-2 md:space-x-3 max-w-[90%] sm:max-w-[85%] ${isUser ? 'flex-row-reverse space-x-reverse' : ''}`}>
        ...
        <div className={`flex-shrink-0 w-8 h-8 md:w-10 md:h-10 rounded-lg md:rounded-xl flex items-center justify-center shadow-md transition-all duration-200 ease-out hover:scale-105 ${isUser 
          ? 'bg-gradient-to-br from-flame-orange to-flame-orange-light ring-2 ring-flame-orange/30 hover:ring-flame-orange/50' 
          : 'bg-gradient-to-br from-purple-dark to-purple-medium ring-2 ring-purple-dark/30 hover:ring-purple-dark/50'
        }`}>
          {isUser ? (
        ...
        <div className={`chat-bubble mobile-bubble-optimized relative group ${isUser ? 'user-bubble mobile-contrast-user' : 'ai-bubble mobile-contrast'} max-w-[calc(100vw-6rem)] sm:max-w-xl`}>
          {/* Indicateur de type de message pour l'IA - Version ultra-compacte pour mobile */}
          {!isUser && (
            <div >
          ...
          <div className="whitespace-pre-wrap mobile-optimized-text leading-relaxed font-medium max-w-full text-sm md:text-base">
            {message.content}
          </div>
          
          {/* Timestamp avec icône et meilleur design - Version ultra-compacte pour mobile */}
          <div className={`flex items-center space-x-1.5 md:space-x-2 mt-2 md:mt-3 ${isUser ? 'justify-end' : 'justify-start'}`}>
            ...
            {!isUser && (
              <div className="flex items-center space-x-1 px-1 md:px-1.5 py-0.5 md:py-1 rounded-full bg-purple-100/80 text-purple-600">
